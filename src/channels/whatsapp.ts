...
import { ASSISTANT_HAS_OWN_NUMBER, ASSISTANT_NAME, DATA_DIR, WHATSAPP_SELF_MESSAGE_GROUPS } from '../config.js';
      ...
      sock.ev.on('messages.upsert', async ({ messages }) => {
        for (const msg of messages) {
          try {
            ...
            // the agent in third-party conversations — except chats (group or
            // DM) explicitly allowlisted via WHATSAPP_SELF_MESSAGE_GROUPS
            // (testing chats where the operator wants their own messages, sent
            // from the bot's shared number, to trigger the agent). Those still
            // rely on sentMessageCache plus the isBotMessage prefix check to
            // skip the bot's own echoes.
            if (fromMe) {
              const isSelfChat = botPhoneJid && chatJid === botPhoneJid;
              const isAllowlistedChat = WHATSAPP_SELF_MESSAGE_GROUPS.has(chatJid);
              if (!isSelfChat && !isAllowlistedChat) continue;
              if (sentMessageCache.has(msg.key.id || '')) continue;
              if (isAllowlistedChat && isBotMessage) continue;
            }
