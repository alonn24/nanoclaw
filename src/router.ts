...
export async function routeInbound(event: InboundEvent): Promise<void> {
  ...
  if (!found) {
    // No messaging_groups row. Auto-create when the message warrants
    // attention: the bot was addressed (@mention or DM), or it's any
    // message in a group chat — groups escalate for approval without
    // requiring a mention, since a group's first message is itself
    // worth surfacing. Plain chatter in non-group channels we merely
    // sit in stays silent — no row, no DB writes.
    if (!isMention && !event.message.isGroup) return;
  ...
  //     escalate to owner for channel-registration approval. Group chats
  //     escalate on any message, same as the auto-create gate above.
  if (agentCount === 0) {
    if (!isMention && !mg.is_group) return;
    if (mg.denied_at) {
